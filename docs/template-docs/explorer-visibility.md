...
@RegisterClass(BaseResourceComponent, 'BizAppsCredentialingOverviewResource')
@Component({
  selector: 'credentialing-overview',
  template: `<div class="p-4"><h2>MJ_BizApps_Credentialing</h2></div>`,
})
export class BizAppsCredentialingOverviewComponent extends BaseResourceComponent {
...
- keep the `startupExport` function (`LoadBizAppsCredentialingClient`) — its only job is to
...
- MJExplorer must import your client package. When the app is **installed**
  ...
  you. In a **linked dev** setup you add the `import '@mj-biz-apps/credentialing-ng';` line
      ...
      "DefaultNavItems": [
        {
          ...
          "ResourceType": "Custom",
          "DriverClass": "BizAppsCredentialingOverviewResource",
          "isDefault": true
        }
      ]
    },
