...
your entity names (`MJ_BizApps_Credentialing: Item Types`) from colliding with MJ core or
...
2. Fill every `TODO`:
   ...
   | `EntityNamePrefix` | Your prefix (e.g. `MJ_BizApps_Credentialing`) — must agree with `mj.config.cjs` `NameRulesBySchema` |
...
```json
{
  "entity": "MJ_BizApps_Credentialing: Item Types",
  "filePattern": "**/.*.json",
  "pull": {
...
`EntityNamePrefix`, e.g. `MJ_BizApps_Credentialing: Item Types`; core entities may need the
...
registered `MJ_BizApps_Credentialing: Item Types`. To seed it: create
