...
> `github.repository`, so the sample `@mj-biz-apps/credentialing-*` packages can never reach
...
stay that way. The template's own `@mj-biz-apps/credentialing-*` packages are **never published**
