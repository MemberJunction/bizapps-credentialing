...
└─ bizapps-credentialing/           <- this repo (rename to your app)
```
...
1. **Server (MJAPI)** — MJ root `mj.config.cjs`:
     ...
     server: [
       {
         ...
         StartupExport: 'LoadBizAppsCredentialingServer',   // must match mj-app.json exactly
   ...
   `"@mj-biz-apps/credentialing-server": "0.0.0"` (whatever version your packages currently
   carry — the template starts at `0.0.0`).

2. **Client (MJExplorer)** — only if you ship UI packages. Add
   `"@mj-biz-apps/credentialing-ng": "0.0.0"` to `packages/MJExplorer/package.json`, and a
   static import to
   `packages/MJExplorer/src/app/generated/open-app-bootstrap.generated.ts`:
   ```ts
   import '@mj-biz-apps/credentialing-ng';
   ```
...
```sh
pnpm run mj:migrate      # mj migrate --schema __mj_BizAppsCredentialing --dir ./migrations
```
...
`pnpm --filter @mj-biz-apps/credentialing-ng run build` (pnpm's filter flag — `npm run
...
2. MJAPI's boot log shows your package loading and `LoadBizAppsCredentialingServer` being
   called.
